...
function [sample_mean, sample_cov] = UT_F(prior_mean, prior_cov, dt, tau_phi, tau_psi, Jsym, old_f)
    ...
    for i = 1:size(sigma_points,2)
        ...
        propagated_sigmapoints(:,i) = double(subs(Jsym.f, old_f, new)); 
    end
...
function [pred_meas, pred_meas_cov, cross_cov] = UT_H(prior_mean, prior_cov, ...
    ...
    for i = 1:size(sigma_points,2)
        ...
        propagated_sigmapoints(:,i) = double(subs(Jsym.h_ut, old_h, new));
    end
    ...
    tilde(2,:) = atan2(sin(propagated_sigmapoints(2,:) - pred_meas(2,1)), cos(propagated_sigmapoints(2,:) - pred_meas(2,1)));
